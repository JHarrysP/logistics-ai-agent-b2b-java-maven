...
public class WarehouseAgent {
    ...
    public WarehouseInstructions generatePickingInstructions(Order order) {
        ...
        if (totalWeight > 1000.0) {
            instructions.append("⚖️ WEIGHT ALERT: Total order weight is ")
    ...
    private int calculateTravelTime(String location) {
        ...
        return switch (zone) {
            case 'A' -> 2; // Zone A is closest to dispatch
            case 'B' -> 4; // Zone B is medium distance
            case 'C' -> 6; // Zone C is farther
            case 'D' -> 8; // Zone D is farthest
            default -> 5; // Default travel time
        };
    }
